    ...
    try {
        ...
        for (let i = 0; i < DAYS_TO_SYNC; i++) {
            ...
            try {
                ...
                console.error(`  Failed sync for ${date.toISOString().split('T')[0]}:`, errorMessage);
        ...
        for (const business of businesses) {
            ...
            try {
                ...
                for (let i = 0; i < ads.length; i += 5) {
                    ...
                    await Promise.all(chunk.map(async (ad) => {
                        try {
                                    ...
                                    leads.map(lead => {
                                        ...
                                        return prisma.lead.upsert({
                                            ...
                                            create: {
                                                ...
                                                full_name: fullName,
                                                phone_number: phone,
                                                raw_data: lead.field_data as unknown as Prisma.InputJsonValue
                                            },
                                            update: {
                                                ad_name: lead.ad_name,
                                                created_time: new Date(lead.created_time),
                            ...
                            if (errorMessage.includes('leads_retrieval') || errorMessage.includes('pages_manage_ads')) {
                                console.warn(`    Permission missing for ad ${ad.id} (Leads)`);
                            } else if (errorMessage.includes('User request limit reached')) {
                                console.warn(`    Rate limit hit for ad ${ad.id} (Leads)`);
                            } else {
                                console.error(`    Failed to sync leads for ad ${ad.id}:`, errorMessage);
                            }
                        }
                    }));
                ...
                const errorMessage = err instanceof Error ? err.message : String(err);
                console.error(`  Failed to sync business ${business.name}:`, errorMessage);
            }
        }
        ...
        console.error("[SmartSync] Critical Error:", errorMessage);
        await updateSyncStatus('failed');
        return NextResponse.json({ success: false, error: 'Sync failed: ' + errorMessage }, { status: 500 });
    } finally {
        setTimeout(async () => {
